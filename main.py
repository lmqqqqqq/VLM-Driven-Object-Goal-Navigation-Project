...
def main():
    ...
    llava_model_name = "./LLaVA/LLaVA-7B-v0"
    
    goal_name = infos[0]['goal_name']
    init_query = "Hi, I am a home assistant robot and I need your help. Now I am in a new house where I have never been before. I know nothing about it and I need to explore it to find a target object. I have a camera on my head to give me constant observations for every step I move. I will send you pictures as observations and you can help me with them. "
    init_query += "For this episode, my mission is to find a "+goal_name+" as soon as possible in this house. First, you need to tell me which of the following type of room do you think I am most likely to find a "+goal_name+": 1. Bedroom, 2. Livingroom, 3. Kitchen, 4. Bathroom. Please give me the index number of the choice."
    
    answer = eval_llava(llava_model_name, None, init_query)
    goal_room_name = answer.split(" ")[-1][:-1]
    ...
    for step in range(args.num_training_frames // args.num_processes + 1):
        ...
        if e==0:
            agent_rgb = obs[e, :3, :, :].permute(1,2,0).reshape(-1,3).cpu().numpy()
            query = "Please give your scores of confidence from 1 to 10. So what is your confidence score of this picture's being a "+goal_room_name+"?"
            answer = eval_llava(llava_model_name, Image.fromarray(agent_rgb), query)
            score = int(answer.split(".").split(" ")[-4])
            
            if score >= 8:
                switch_identify = True
        
        if switch_identify == True:
            for e in range(num_scenes):
